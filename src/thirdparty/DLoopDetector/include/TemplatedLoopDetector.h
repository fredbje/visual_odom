...
#include "DBoW2/include/TemplatedDatabase.h"
#include "DBoW2/include/QueryResults.h"
#include "DBoW2/include/BowVector.h"
...
struct DetectionResult
{
    ...
    EntryId match;
    /// Fundamental matrix between match and query. Empty if no match. P_match' * F * P_query = 0
    cv::Mat F;
    /// Feature vector of query image
    cv::Mat queryFeatures;
    /// Feature vector of matched image
    cv::Mat matchFeatures;
    /// inliers (out) vector s.t. inliers[i] == 1 if i-th point is an inlier, 0 otherwise
...
protected:
    ...
    * @param match (out) returns fundamental matrix, query and match feature vectors and inlier vector
    */
    bool isGeometricallyConsistent_DI(EntryId old_entry,
                                      const std::vector<cv::KeyPoint> &keys,
                                      const std::vector<TDescriptor> &descriptors,
                                      const FeatureVector &curvec,
                                      DetectionResult &match) const;
    ...
    * @param match (out) returns fundamental matrix, query and match feature vectors and inlier vector
    */
    bool isGeometricallyConsistent_Exhaustive(const std::vector<cv::KeyPoint> &old_keys,
...
template<class TDescriptor, class F>
inline const TemplatedDatabase<TDescriptor, F>& TemplatedLoopDetector<TDescriptor, F>::getDatabase() const
{
    return *m_database;
}
...
template<class TDescriptor, class F>
inline const TemplatedVocabulary<TDescriptor, F>& TemplatedLoopDetector<TDescriptor, F>::getVocabulary() const
{
                        ...
                        if(getConsistentEntries() > m_params.k)
                        {
                            ...
                            else if(m_params.geom_check == GEOM_FLANN)
                            {
                                ...
                                detection = isGeometricallyConsistent_Flann(island.best_entry,
                                        keys, descriptors, flann_structure, match);
                            }
...
template<class TDescriptor, class F>
double TemplatedLoopDetector<TDescriptor, F>::calculateIslandScore(const QueryResults &q,
...
template<class TDescriptor, class F>
bool TemplatedLoopDetector<TDescriptor, F>::isGeometricallyConsistent_DI(EntryId old_entry,
                                                                         const std::vector<cv::KeyPoint> &keys,
                                                                         const std::vector<TDescriptor> &descriptors,
                                                                         ...
                                                                         DetectionResult &match) const
{
    ...
    if((int)i_old.size() >= m_params.min_Fpoints)
    {
        ...
        if(m_fsolver.checkFundamentalMat(oldMat, curMat, m_params.max_reprojection_error, m_params.min_Fpoints,
                                             m_params.ransac_probability, m_params.max_ransac_iterations)) {
            match.queryFeatures = curMat;
            match.matchFeatures = oldMat;
            match.F = m_fsolver.findFundamentalMat(oldMat, curMat, m_params.max_reprojection_error, m_params.min_Fpoints,
                                                &match.inliers, true, m_params.ransac_probability,
...
isGeometricallyConsistent_Exhaustive(const std::vector<cv::KeyPoint> &old_keys,
                                     const std::vector<TDescriptor> &old_descriptors,
                                     const std::vector<cv::KeyPoint> &cur_keys,
                                     const std::vector<TDescriptor> &cur_descriptors,
                                     DetectionResult &match) const
...
template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::getFlannStructure(const std::vector<TDescriptor> &descriptors,
                                                              cv::FlannBasedMatcher &flann_structure) const
{
                                                                            ...
                                                                            DetectionResult &match) const
{
    ...
    for(int old_idx = 0; old_idx < (int)matches.size(); ++old_idx)
    {
        if(!matches[old_idx].empty())
        {
            ...
            if(ok)
            {
                vector<unsigned int>::iterator cit = std::find(i_cur.begin(), i_cur.end(), cur_idx);
...
template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::getMatches_neighratio(const vector<TDescriptor> &A,
...
template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::removeLowScores(QueryResults &q, double threshold) const
{
